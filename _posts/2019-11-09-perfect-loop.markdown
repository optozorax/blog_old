...
Как вы знаете, я люблю рисовать фракталы, а ещё больше я люблю рисовать дерево Пифагора. Однажды я захотел сделать модную гифку, где камера будет двигаться таким образом, как будто мы, вращаясь и уменьшаясь, бесконечно погружаемся внутрь фрактала, при этом гифка должна быть идеально зацикленной, то есть что-то такое:

![](/assets/interpolation/p3_4.2_45.gif)

В данной статье расскажу с чего я начал, на какие подводные камни наткнулся и как мне пригодилось взятие функции от матрицы - одна из сложнейших тем в универском курсе линейной алгебры.

# Рисование фрактала

Как рисовать дерево Пифагора, я рассказывал в [предыдущей статье.](/space-objects)

# Как сделать эту гифку

Для получения этой гифки вместо того, чтобы двигать фрактал, я буду двигать камеру вокруг фрактала:

![](/assets/perfect-loop/move_cam.gif)

А чтобы как-то задавать камеру, я буду задавать систему координат, привязанную к этой системе координат.

Для начала: что будем двигать?

Посмотрите на картинку. Видите систему координат? Я сделал так, чтобы прямоугольник камены задавался локальными координатами: (-2, -0.5), (2, 2.5).

![](/assets/perfect-loop/start.png)

Например, возьмём вторую систему координат в произвольном месте:

![](/assets/perfect-loop/coords.png)

И нарисуем как будет выглядеть прямоугольник экрана для этой системы координат:

![](/assets/perfect-loop/coords_with_screen.png)

И как будет выглядеть изображение из этого прямоугольника:

![](/assets/perfect-loop/start_from_coords.png)

Таким образом задано однозначное соответствие между системой координат и положением камеры, теперь переформулирую задачу с точки зрения координат:

Итак, у нас есть первая система координат и вторая, наша задача состоит в том, чтобы придумать такую интерполяцию между этими системами координат, чтобы в итоге получилась идеально зацикленная гифка.

# 2д камера

Ещё задача заключается в том, чтобы выбрать такое внутренне положение, чтобы при интерполяции от него к следующему, мы не видели как что-то появляется, а что-то исчезает. Сравните:

*изображение, где дерево пифагора начинается с самого начала, а не сглубины 1*

Например, тут я выбрал слишком большую глубину:

https://twitter.com/optozorax/status/1124273102018437122

Так же я не описал такую проблему как то, что при приближении деталей меньше, чем при дальнем отображении, решаю это постепенным появлением деталей во время движения, просто меняю прозрачность:

гифка

Ещё я не описал то, что прямоугольник для камеры я получаю не вручную, а следующим образом:

рассчитываю все координаты дерева пифагора в памяти, перебираю их все, и нахожу максимум среди координат и миниум, таким образом я получаю прямоугольник, который включает в себя все координаты этого фрактала полностью и без зазоров. Затем увеличиваю этот прямоугольник на 20%, чтобы получилась граница
